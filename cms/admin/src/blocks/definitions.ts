...
export const blockDefinitions: BlockDefinition[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'showTimeline', label: 'Show the timeline rail (needs 2+ group headings)', type: 'switch' },
      { key: 'railColor', label: 'Timeline line & dot colour (ว่างไว้ = ตามสีตัวอักษร)', type: 'color' },
      { key: 'starColor', label: 'Star colour', type: 'color' },
      { key: 'headingColor', label: 'Heading colour', type: 'color' },
      { key: 'textColor', label: 'Text colour', type: 'color' },
      { key: 'background', label: 'Background colour', type: 'color' },
    ],
  ...
  },
  {
    ...
    defaultProps: {
      ...
      closedIcon: '',
      links: [],
      footerText: 'STREAM TOGETHER ON STATIONHEAD\nEVENT DURATION : 25.09.26 - 25.10.26',
    },
    fields: [
      { key: 'heading', label: 'Title (one line per line)', type: 'textarea' },
      { key: 'logo', label: 'Logo above the title', type: 'image' },
      { key: 'mascot', label: 'Sticker beside ON AIR (e.g. the tooth)', type: 'image' },
      { key: 'fallbackPhoto', label: 'Main photo on the left (the artist)', type: 'image' },
      { key: 'onAirLabel', label: 'On-air badge text', type: 'text' },
      { key: 'onAirIcon', label: 'ON AIR sign picture (replaces the drawn badge; optional)', type: 'image' },
      { key: 'offAirIcon', label: 'OFF AIR sign picture, when nobody is on (optional)', type: 'image' },
      { key: 'openIcon', label: 'Calendar OPEN sign picture (optional)', type: 'image' },
      { key: 'closedIcon', label: 'Calendar CLOSED sign picture (optional)', type: 'image' },
      { key: 'streamUrl', label: '▶ STREAM NOW button (beside ON AIR) — link to the streaming app. Blank hides the button', type: 'url' },
      { key: 'streamLabel', label: '▶ STREAM NOW button (beside ON AIR) — text', type: 'text' },
      { key: 'nextLabel', label: 'Next-up label', type: 'text' },
      { key: 'accentColor', label: 'Neon colour', type: 'color' },
      { key: 'showNext', label: 'Show who is up next', type: 'switch' },
      {
        key: 'links',
        label: 'Footer band icons (the red strip at the bottom — not the STREAM NOW button)',
        type: 'items',
        itemFields: [
      ...
      { key: 'footerText', label: 'Footer text (one line per line)', type: 'textarea' },
    ],
  },
  {
    type: 'song',
